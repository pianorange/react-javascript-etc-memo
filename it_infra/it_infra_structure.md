...
-> 블록단위로 증감 데이터만 복제 위치에 반영해서 데이터 전송량 줄임  
...
- 복제 서버 많으면 갱신이 많은 시스템과 같이 복제 오버헤드 높아짐(복제에 필요한 리소스가 많아진다고)
- 실제 데이터와 복제 데이터 완전히 일치시키고 싶으면 복제 데이터의 쓰기 완료 처리 보장해야 함. (시스템 응답 악화 가능성)
- 시스템 유지관리, 장애 시에는 복제 데이터도 고려해야 되서 설계, 운용 난이도 높아질 수 있음

### 마스터- 슬레이브

마스터 한대가 슬레이브 일괄관리 하는구조  
-> 마스터가 다수의 슬레이브관리하므로 상대적으로 마스터 부하 높아지는걸 피할 수 없음  
반대는 peer-to-peer: 모든 서버 대등하며, 각자 관리하는 구조

장점

- 마스터 한대이기 때문에 구현이 쉽다
- 슬레이브 간 처리 동기화할 필요 없어 통신량 줄어든다.
...
디지털 데이터의 쓸데없는 공간 줄이기 위해 사용.

디지털 데이터 압축의 기본은 '중복 패턴 인식'과 그것을 '변경'하는 것.

- zip 파일 압축 시 어떤 파일은 크기가 작아지고 어떤 파일은 그대로  
  -> 같은 패턴이 어느 정도 있냐에 따라 압축률 달라지기 때문  
  -> 같은 패턴 많을 수록 압축률 높아진다.  
  -> JPEG등 이미지 파일은 이미 압축돼 있어 중복 패턴 거의 없는 상태라, 압축해도 크기 변화가 없다.

- 가역 압축: 원래 상태로 복구할 수 있는 압축  
  -> 이미 알고있는 정보를 제거하는 압축방식
- 비가역 압축: 원래 상태로 복구할 수 없는 대신 압축률 높은 방식  
  -> 자신에게 필요 없는 정보를 필요 최소로 제거하는 방식

- 자바의 jar, war은 실제로는 zip 형식 압축파일.  
  -> jar는 사실 압축이라기 보다 하나로 모은다는 의미가 강함

- DB나 저장소서버는 압축시간 때문에 쓰기 속도 떨어지면 곤란하기 때문에 비교적 간단한 압축구조 사용하는 경우 많음.

- 중복 제거(디듀플리케이션 deduplication)가 대표적인 예시  
  -> 기록할 데이터 일정 단위 블록으로 처리해 이전에 입력된 적 있는 중복된 블록은 제거.  
  -> 데이터 기록시 블록의 해시값을 계산해서 아직 기록되지 않은 블록은 저장하고 기록된 것은 저장하지 않는 방식

## 5.9 오류 체크/오류 수정

데이터 통신중 데이터 무결성 체크에 관한 이야기

#### 디지털 데이터 오류 생기는 원인 예시

1. 통신중 데이터 파손  
   대부분 통신방식 전기 신호 이용 데이터 교환하는데, 도중에 낙뢰로 전기적 잡음이 섞여 데이터가 파손된 경우

2. 칩에서의 데이터 파손  
   메모리의 데이터도 전기적으로 저장되므로 전기적 영향에 의해 데이터 파손가능성 있음

### 5.9.3 실수나 오류를 검출하려면

데이터 변질이 없었는지 알기 위해 추가 정보를 사용  
정답은 없다 단지 변질됬는지 무슨 수단을 써서라도 확인할 수 있으면 됨.

- 오류검출방법의 예시

서버용 메모리의 경우:  
패리티 비트(parity bit)라는 추가 비트 부여하는 방법

네트워크 통신의 경우:  
프로토콜에 따라 각 계층의 오류 검출용 구조 갖추고 있음.  
ex)  
TCP/IP프로토콜 및 이더넷은 계츨별로 체크섬이나 CRC오류 검출 구조 도입하고 있음.  
-> 이 덕에 수신 측에서는 해당 프레임이나 패킷에 오류가 있다는 것을 감지하면 해당 데이터 제외.  
-> TCP 사용 중이라면 자동으로 TCP가 빠진 패킷 재전송 요구

# 6. 시스템을 연결하는 네트워크 구조

## 6.2 계층 구조란?

계층구조 에서는 데이터나 기능 호출 흐름에 다라 계층간 역할이 나누어진다는 특징 있음.  
역할이 나누어져 있기 때문에 각 층은 자신이 담당하는 일만 책임 지며 다른 일은 다른 계층이 책임을 진다.

장점

- 은폐화, 각 계층 독립적으로 동작 보장하므로 인터페이스만 유지하면 내부처리 바뀌어도 문제 없음
- 단점, 작업 효율 희생해야 함. 여러 계층에 작업 인계를 위한 오버헤드가 발생함

### 6.2.3 계층 모델의 대표적인 예 OSI 7계층 모델

OSI(Open Systems Interconnection) 통신규역 만들때 고안된 것  
OSI 통신 기능을 7개의 계층으로 나눈 것  
-> OSI자체는 지금 사용되지 않지만 구조개념은 다양한 분야에서 공통적으로 참조하는 '참조 모델'로 사용되고 있다.

![OSI7layer](./it_infra_img/OSI7layer.jpg) 7. Application layer: 애플리케이션 처리  
6. Presentation layer: 데이터 표현 방법  
5. Session layer: 통신 시작과 종료 순서  
4. Transport layer: 전송 계층, 통신 관리  
3. Network layer: 네트워크 송신 경로 선택  
2. Data link layer: 직접 접속돼 있는 기기 간 처리

1. Physical layer: 전기적인 접속

## 6.3 프로토콜이란?

`protocol` 이란 단어는 '사전에 정해놓은 순서 의미'  
->컴퓨터가 소통하기 위해 정한 규약

### 6.3.1 사람끼리의 의사소통도 프로토콜

- 언어(영어, 한국어) = 사람끼리 통신 위한 프로토콜  
  -> 매체(깃발, 수신호, 신문 등)도 프로토콜

ex)
한국어로 대화를 한다고 치면,  
-> 음성 통신 프로토콜(매체) 위에 한국어 통신 프로토콜을 올려 놓은 것과 같다.

### 6.3.2 컴퓨터에서는 프로토콜이 필수 불가결

컴퓨터 통신 프로토콜도 사람의 의사소통처럼  
`통신매체`와 그 위에 흐르는 `의미`를 나누어 생각할 수 있다.

ex)
브라우저로 웹페이지 볼 때  
-> `의미`HTTP프로토콜 사용 서버에게 웹 페이지 요청  
브라우저 GET/xx.jpg HTTP/1.0 <-> HTTP/1.0 200 OK 웹서버  
-> `통신`은 전기 신호나 전파 이용해서 전달

컴퓨터도 제조사에 따라 대화하는 언어를 일치시켜야 통신 가능  
때문에 네트워크 업계에서는 공통 프로토콜 선정 위한  
IEEE나 IETF같은 표준화 단체가 존재

### 표준화 단체(IEEE나 IETF)

프로토콜은 약속. 모두가 대응하지 않으면 의미가 없다.  
-> 표준화 필요  
`IEEE`는 전기전자 기술 학회지만, 전기통신 프로토콜 표준화 활동도 하고있다.  
-> 그중 우리가 자주 접하는것이 무선 LAN프로토콜 IEEE802.11n대응

`IETF` 인터넷 관련 다양한 기술 표준화 하는 단체  
`RFC Request for Comment`문서 주기적으로 발표  
-> 인터넷 프로토콜의 기본적인 부분은 RFC 791에서 정하고 있음

### 6.3.3 프로토콜은 서버 내부에도 존재

프로토콜하면 TCP/IP를 주로 떠올릴 텐데, 어떤 장비라도 서로 통신하기 위해 프로토콜이 필요하다.  
EX)  
마우스를 PC에 연결할 때 사용하는 USB  
-> USB프로토콜 존재  
저장소나 CPU의 멀티코어간 통신하기 위한 프로토콜도 존재

## 6.4 TCP/IP

인터넷 포함 현재 네트워크를 지탱하는 것은 TCP/IP 및 관련 프로토콜 이것들 모아서 TCP/IP 프로토콜 슈트라고 한다.

### 6.4.1 인터넷의 발전과 TCP/IP프로토콜

1. 1969: 인터넷 시초 미 국방성 연구. Arpanet 아파넷
2. 1980년대: 네트워크 제조사별로 수많은 독자 프로토콜 사용 상호접속 문제 있었음.  
   -> 1982 국제 규격 프로토콜 만들자는 움직임 시작되 OSI프로토콜 재정
   -> OSI 사양 너무 복잡해 사장되고, 1970년대 고안된 TCP나 IP프로토콜이 계속 퍼져 사실상 인터넷 표준이 됨

### 6.4.2 TCP/IP 계층 구조

TCP/IP 프로토콜 슈트:  
명칭 그대로 TCP와 IP 두가지 프로토콜 주축으로 한 프로토콜 집합  
->TCP반드시 사용해야 하는것은 아니며, IP는 통신 상대 지정시에 IP주소 이용하기 때문에 반드시 필요

### OSI 7계층 과 TCP/IP

TCP/IP이 반드시 7계층으로 분명하게 나뉘지는 않음

- TCP/IP 4계층 or 5 계층으로 나누는게 일반적
- OSI 7계층의 1~2계층 모아서 링크계층
- OSI 7계층의 5~7계층 모아서 애플리케이션으로 취급하기도 함.

![TCP/IP4layer](./it_infra_img/TCPIP4layer.png)

- 애플리케이션 이미지, 텍스트 등 전달하기 위해 HTTP프로토콜 사용
- HTTP 통신 데이터 전송하기 위해 전송계층인 TCP 프로토콜에 데이터 건네고 여기서부터 이더넷 계층 까지는 OS커널이 담당
- 커널 내에서 TCP, IP, 이더넷 담당하는 기능이 필요한 정보를 데이터에 부여해서 최종적으로 이더넷 프레임이 생성.  
  -> 이것이 NIC(랜카드)에 전달되서 이더넷 케이블 등을 통해 인접 노드 경유해서 최종위치까지 전달 됨

TCP/IP 4계층 모델 (HTTP통신 예)  
4. 애플리케이션 계층(HTTP) :이미지 데이터를 보내고 싶어 송신은 다른계층에 맡기자  
3. 전송계층(TCP): 애플리케이션이 의뢰한 데이터는 책임지고 상대방에 전달한다  
2. IP 계층(IP):데이터를 최종 위치까지 운반할게!

1. 링크 계층(이더넷): 직접 연결되있는 주변장비에게도 보내자!

- 계층구조로 나뉘어 있어 애플리케이션은 독자적으로 통신 구조 만들 필요 없이 TCP/IP에 위임 가능
- 각 계층 담당레이어 별로 세부사항 변경 용이(인터페이스만 맞춰주면 됨)  
  ex )
- TCP 대신 신뢰성 낮지만 간단히 데이터 송수신 가능한 UDP로 변경
- 유선통신에서 무선통신으로 변경

- 현장에선 TCP/IP 4계층을 OSI 7계층으로 혼용해서 부르는 경우가 많다.

레이어2: 링크계층(이더넷 계층)  
레이어3: IP계층
레이어4: TCP계층  
각 계층별 처리를 위한 네트워크 스위치 : L2스위치, L3스위치

레이어7: 애플리케이션 계층.(5,6,7계층) 모아서 애플리케이션 계층으로 취급하므로 TCP/IP이야기 할때는 묶어서 이야기 함

## 6.5 레이어7 HTTP 애플리케이션 계층의 프로토콜

애플리케이션이 없으면 통신이 시작되지 않지.  
애플리케이션이 사용하는 프로토콜을 애플리케이션 계층프로토콜이라 부른다.  
-> 이때 통신 자체는 모두 TCP/IP에 맡김

- 브라우저에 URL입력해서 요청이 웹서버에 도달하면 응답으로 HTML파일이 반환된다.
- 브라우저는 이 파일 해석, 파일 내에 추가 이미지 스크립트 등 있으면 다시 웹서버에 이들 요청.  
  ->HTTP를 통해서 클라이언트와 웹서버는 몇번이고 요청과 응답 반복

### 6.5.1 요청과 응답의 구체적인 내용

HTTP는 통신제어 관여 하지 않으며 요청과 응답은 여러가지 명령만으로 구성돼서 매우 간단한 형태를 띈다.

#### Request

`요청`: 서버에 대한 요구 명령, 대상 데이터 지정.  
-> GET, POST/HTTP1.1  
`메시지, 헤더:` 브라우저에 상세정보 전달  
->User-Agent: 브라우저 식별 정보(버전,브라우저 이름)  
->HOST: www.aaa.~~
-> Cookie: 세션 식별자  
`메시지 바디`:  
브라우저에 입력한 내용 포함됨
...
`상태`: 응답 상태가 정상인지에 관한 정보  
-> HTTP/1.1 200 OK  
`메시지, 헤더`: 브라우저에 상세 정보 전달(표시대응 브라우저, 데이터 형식, 접속 방식 등)  
User-Agent: Mozilla/4.0  
Content-Type: text/html  
Connection: keep-alive  
cookie: xxxxx  
`메시지, 바디`: HTML데이터 등 실제 데이터가 저장

- 애플리케이션 계층 프로토콜인 HTTP를 통해 아래의 프로세스들이 통신

<클라이언트>

1. 클라이언트 프로세스는 HTTP형식으로 응답 작성해 `소캣`에 데이터를 기록.

<TCP/IP> 2. `소캣`에 기록된 데이터는 다른 한쪽 소켓으로 전달 됨.  
-> 클라이언트의 애플리케이션 자체가 통신 구조 가지지 않고서도 원격지에 있는 서버 애프리케이션과 통신 가능하다는 것을 의미.

<웹서버> 3. httpd프로세스 `소캣`을 통해 통신 상대가 보낸 데이터 읽어서 통신 상대의 요청 등을 처리.

### 6.5.3 소켓 이하는 커널 공간에서 처리

`소켓`은 어떻게 만들어진 것일까?

1. 클라이언트에서 애플리케이션 프로세스 네트워크 통신 하는 경우,  
   시스템 콜 통해 커널에 TCP/IP 통신하고 싶으니까  
   통신할 수 있는 회선 열어달라고 의뢰.  
   -> 이때, 접속 대상 서버의 `IP 주소`와 `TCP포트` 필요.  
   -> 각각 `IP 도착 위치`와 `TCP 도착 위치`

2. 의뢰받으면 커널이 소켓을 만들어줌(데이터 보내기 위한 입구 만들어줌)
3. 소켓만들어 지고 나면, 커널은 IP 주소 및 포트 번호 이용해 접속 대상 서버와 연결을 생성.  
   -> 이때 상대 서버에도 소켓 만들어지며 상대 서버와 연결된 가상경로(`버추얼 서킷`)가 생성 됨.  
   -> 물리적인 흐름은 커널경유해서 NIC를 통해 데이터 전송되면 다양한 경로 거쳐 전송지의 소켓에 데이터 전송. 전송지 커널은 데이터를 받아 프로세스에 넘긴다.

#### Keep-Alive

HTTP는 한번 요청/응답 관계가 끝나면 세션을 종료한다.
-> 즉, HTTP는 Connectionless 방식으로 연결을 매번 끊고 새로 생성하는 구조이다.

이는 Network 비용 측면에서 최초 연결을 하기 위해 많은 비용을 소비하는 구조이다.  
이 때문에 한 화면에 많은 이미지가 포함 되 있으면 몇번이고  
세션을 만들고 닫는 처리를 해야한다  
-> 결국, 하위 계층인 TCP에서 3way-handshaking 등 의 오버헤드 발생해 전체 화면 응답에 영향 끼침.

이 때문에 Keep-Alive 사용해 화면 하나 표시할 정도의 세션을 유지 하는 설정이 필요.  
-> Keep-Alive는 HTTP/2에서 무시 됨  
-> Telnet(연결)은 한 번만 이뤄졌고 설정한 timeout, max 설정 만큼 연결 세션(connection)을 유지한다.
-> handshake 즉 세션 새로 만드는 비용이 절약됨

어느정도 세션 유지할 지는 화면 하나당 평균 응답시간으로 계산.  
무의미하게 세션 길게 유지하면 HTTP 프로세스/응답이 세션을 놓지 못해서 신규요청 접수할 수 없다.

## 6.6 레이어 4 전송 계층 프로토콜 TCP(Transmission Control Protocol)

### TCP 임무:

1. 애플리케이션이 보낸 데이터 그 형태 그대로 상대방에게 확실히 전달 하는 것.
2. 데이터를 `서버가 송신할 때`와 `서버가 수신한 후` 애플리케이션에게 데이터 전달할 때 TCP가 움직임.  
   -> 데이터 도착순서, 전송성공 여부 확인 기능 등
   -> 상대 서버까지 전송은 하위 계층인 IP에 모두 위임.  
   -> IP 만으로도 통신 가능하지만 IP에는 데이터 확실히 전달됐는지 확인하는 기능이나 도착한 순서 확인하는 기능 등이 없다.

ex) 100장의 자료를 봉투10개에 나눠 순서대로 보냈다고 치면,
도중에 빠진 봉투는 없는지, 자료의 순서는 어떤지 확인할 방법이 필요하지. IP 만으로는 이게 안됨
...
다양한 기관이 관리하는 네트워크가 대규모 인터넷 구성하기 때문에 보낸 소캣이 인터넷 경로 어딘가에서 행방불명되거나, 네트워크관련 문제로 패킷이 훼손 되는 등 장애 요소가 많다.
-> 그래서 양자(end-to-end)간 신뢰성 담보할 TCP같은 구조 필요.

### 6.6.2 커널 공간의 TCP처리 흐름

- TCP는 세그먼트(segment)라는 단위로 데이터를 관리한다.
-

1. 애플레케이션 프로세스 소켓에 데이터 기록
   ->시스템 콜 인터페이스 경유
2. 소켓에 기록된 데이터는 소켓의 큐에 리스팅 됨
3. 커널에 전달된 데이터 '소켓 버퍼'라는 메모리 영역에서 처리 됨
   -> 데이터에 TCP헤더 붙여서 TCP 세그먼트가 생성됨.  
   -> TCP 헤더에는 도착 지점의 포트번호 등 TCP기능에 필요한 다양한 정보 기록되있음  
   -> 하나의 TCP 세그먼트로 전송할 수 있는 최대 크기를 MSS 라고 함. (MSS Maximum Segment Size)
   -> 크기 초과시 데이터 자동적으로 분할 되 복수의 TCP세그먼트 생성  
   -> But 전송은 링크계층에 의존하기 때문에 MSS 사이즈도 링크계층 전송가능한 최대 사이즈에 의존한다.

### TCP 세그먼트분할

대부분의 환경에서 MSS(Maximum Segment Size)는 1460바이트

예를들어  
2000바이트 데이터가 애플리케이션에 의해 소켓에 기록됬다고 치면,

데이터 2000  
MSS 제한 1460  
->데이터 분할 됨 1460 + 540
-> 각각의 데이터에 TCP헤더가 붙어 2개의 TCP세그먼트생성
-> TCP헤더에는 TCP의 다양한 기능을 위한 수많은 정보가 담김  
ex) 전송 출발지 포트, 전송 목적지 포트, 시퀀스번호, 체크섬 etc

### 6.6.3 포트 번호를 이용한 데이터 전송

- 상대 서버에 데이터 도착하면 TCP는 도착한 TCP세그먼트의 포트번호 이용해서 어떤 어플리케이션에 데이터 전달할지 판단.

### 6.6.4 연결 생성

- TCP는 연결형 프로토콜이며, 연결connection이라 불리는 가상경로(버추얼 서킷)생성.

- 연결connection(버추얼 서킷)은 TCP 통신 시작하며, 통신 상대에게 '지금부터 통신한다'라고 연락 후 상대에게 OK 사인 받으면 생성됨.
- 요청 받는 서버 프로세스는 특정 포트번호로 접속이 요청되었을때 자신에게 연결 되도록 'Listen'상태여야 한다.

### 3-way handshanking

이하의 3 과정을 일컫는 단어

1. 클라이언트 통신상대인 서버측 OS에게 포트번호와 함께 가상경로 열도록 의뢰
2. 서버측에서는 리슨중인 포트번호로 통신요구가 오고, 문제없으면 OK응답.
3. 클라이언트측도 확인했다는 메시지 보내면 통신용 가상 경로 열린다

### 6.6.4 데이터 보증과 재전송 제어

TCP의 기능에는 데이터가 확실히 전달되도록 보증하는 기능이 있다.

### 데이터 손실 방지기능:

확인 응답과 재전송에 의해 구현 됨.

- 수신 측 TCP 세그먼트 도착하면 수신 측은 송신 측에게 도착했다는 것 알림.  
  ->이때 반환 하는 것 ACK이며 송수신 왕복 줄이기 위해 TCP 헤더에 포함시켜서 반환 (핑퐁)
- 송신 측 돌아온 ACK 보고 전송한 세그먼트 무사히 도착했다고 확인.
- ACK돌아오지 않으면 TCP세그먼트가 어떤 이유로 사라졌을 가능성 있음  
  -> 언제든 재전송 가능하도록 전송 끝난 TCP세그먼트라도 ACK가 돌아올때까지 소켓 버퍼에 남겨 둘 필요가 있다.

#### 송신측 재전송 판단 방법

1. 타임아웃  
   일정시간 내에 ACK 돌아오지 않으면 재전송
2. Duplicate ACK (중복 ACK)
   똑같은 ACK번호(sequesce번호붙어있어 순서파악가능)가 중복돼서 도착한 경우 그 번호에 해당하는 TCP 세그먼트 도착하지 않았다 판단해서 재전송
   -> 전송중 지연 가능성 고려해 3회 중복까지는 기다려줌

#### 데이터 순서 보증하는 구조:

각 TCP세그먼트에 sequence 번호 붙여서 구현.  
-> TCP 헤더에 기록됨  
-> 해당 TCP 세그먼트가 가지고 있는 데이터가 전송 데이터 전체 중 몇 바이트부터 시작하는 부분인지 가리킴.  
-> 수신측은 ACK 반환시 송신측이 참조하라고 다음에 필요한 TCP 세그먼트의 시퀀스 번호도 ACK번호로 전달.  
ex) 3000바이트 1460, 1460, 80 3개 세그먼트로 쪼개지면  
첫째 세그먼트 번호가1, 두번째가 1461 마지막이 2921이 되는 방식

- TCP는 이 secquence 번호 사용해서 데이터를 원래 순서대로 조립

#### 흐름 제어와 폭주 제어

시퀀스 번호와 ACK를 이용해 데이터 보증을 편리하게 할 수 있지만 데이터를 보내고 ACK기다리는 처리 반복하면 시간이 많이걸림.  
-> 이 내용만 두고 보면 `동기방식 처리` 지만 실제로는 윈도우 라는 개념 이용해 `비동기방식 처리`

### 흐름제어

TCP는 일정 데이터 크기까지라면 ACK기다리지 않고 TCP 세그먼트를 전송하는 윈도우 개념을 가지고 있다  
-> TCP의 윈도우: ACK 기다리지 않고 전송 가능한 데이터 크기

-> 윈도우에는 수신측의 수신 윈도우와 송신측의 폭주(송신) 윈도우 2가지  
-> 데이터 송수신 상황에 따라 윈도우를 이용해 흐름을 제어

ex)

1. 수신측 수신윈도우: 8540바이트(세그먼트 4개분)까지는 한번에 받을 수 있어
2. 송신측 폭주윈도우: 8540바이트(세그먼트 4개분)까지는 ACK응답 없이도 보낼게! ACK 돌아옴에 따라 다음 세그먼트로 폭주윈도우 이동 (이렇게 이동하는 방식 sliding window라고 함)

### 폭주제어

송신 측 윈도우를 `폭주 윈도우`라고 부르는 이유  
송신 측 윈도우 크기를 네트워크 폭주 상태(혼잡 상태)에 맞추어 변경시키기 때문.  
네트워크 혼잡 -> 윈도우 크기 축소해 전송 데이터 양 줄임

폭주 윈도우 크기는 통신 시작시 1세그먼트에 설정  
-> ACK반환 시마다 폭주 윈도우 크기 2세그먼트,4세그먼트 식으로  
지수함수적으로 늘려나가다 어느정도 까지 늘어나면 1세그먼트씩 크기 늘려나감  
-> 네트워크 혼잡 감지하면 다시 속도 줄이고 늘리기 반복

## 6.7 레이어3 네트워크 계층의 프로토콜 IP

TCP 세그먼트 만들어 지면 다음은 IP 처리 시작 됨

IP(Internet Protocol)  
인터넷에서 사용되는 가장 중요한 프로토콜  
가장 폭넓게 사용 되는 것은 IPv4  
->새로운 버전인 IPv6도 있지만 기업형 시스템 등 아직 IPv4가 대세

### 6.7.1 IP의 역할

'지정한 대상 서버까지 전달받은 데이터를 전해주는 것'  
-> 단, IP에서는 반드시 전달된다는 것을 보장하지 않는다.
...
### 6.7.2 커널 공간의 IP처리 흐름

생성된 TCP 세그먼트는 그대로 IP처리에 돌입.  
IP계층에서는 최종 목적지 적힌 IP헤더를 TCP 세그먼트에 추가해서 IP 패킷 생성
...
- IP헤더(IPv4헤더) 기본적으로 20바이트
  -> TCP 헤더 또한 기본적으로 20바이트
  -> TCP에 의해 세그먼트는 전송 최대크기 MSS인 1460바이트로 쪼개지므로 IP 패킷은 1460 + 20(TCP헤더)+20(IP헤더)

- Ip에도 TCP 세그먼트 분할 기능 처럼 패킷 분할 기능 존재  
  -> 패킷 분할 = 조각화(fragmentation)  
   패킷 재결합 기능 = 재결합reassemble

### IP 주소 이용한 최종 목적지로의 데이터 전송

IP주소는 대상서버를 나타내는 주소.  
32비트 표현된 숫자 집합이며, 컴퓨터 처리 숫자이기에 이진수 써야 하지만 사람이 읽기 쉽도록 8비트 단위로 마침표 찍어서 표현  
각각의 영역을 옥텟이라고 부르며 1개의 옥텟에 1byte의 크기를 가집니다. 0~255까지 표현이 가능하며 총 4byte(32bit)의 크기를 가집니다.

192.168.0.1  
32비트로 표현된 숫자 집합을 8비트 단위로 마침표니까  
이진수로 바꾸면 8자리갖는다.
11000000.10101000.00000000.000000001

- IP 주소는 네트워크부와 호스트부로 나뉨 (어디의, 누구인지)
  -> 네트워크부: 어떤 네트워크인지  
  -> 호스트부: 해당 네트워크 내에 있는 컴퓨터(HOST) 가리킴

- 네트워크 부 표시하기 위해'/24'와 같은 CIDR(사이더)표기 사용하거나 '서브넷 마스크'라고해서 255.255.255.0과 같이 표현하기도 한다.  
  -> /24 와 255.255.255.0는 같은 의미 (표기방식이 다른거)  
  -> /24 (CIDR표기)를 서브넷 마스크 방식으로 생각하면  
  좌측부터 24개가 1이고 나머지 0이라는 의미  
  255.255.255.0를 이진수로 바꾸면  
  255 는 이진수로 1이 8개 x 3 = 24개  
  11111111.11111111.11111111.000000000

- 같은 네트워크 내 컴퓨터 IP주소는 네트워크부를 동일 값으로 설정해야 한다.  
  -> 전화번호에서 지역번호 02-XXX-XXXX존재하듯 같은 네트워크를 나타내는 주소가 있음
  -> 서브넷 마스크는 IP주소중 몇번째까지가 네트워크부인지 나타내기 위한 표기

- CIDR 표기 /24면 이진수 표기에서 24번째 숫자까지 네트워크부가 된다.

- IP 주소 중 호스트부 비트가 모두 0인것은 네트워크주소, 1인것을 브로드캐스트 주소. 위 둘은 특별한 주소이므로 호스트에 할당해선 안된다.
  ex) 192.168.8.0 네트워크주소
  192.168.8.255 브로드케스트주소주소

- 브로드캐스트 주소로 보낸 패킷은 같은 네트워크의 모든호스트에 전달 됨

#### MEMO 서브넷 마스크 표기법

보편적으로 CIDR(Classless Inter-Domain Routing)의 유연성있는 서브넷 마스크 표기법을 이용하여 표현합니다.
...
이에 자릿수 늘린 IPv6등장.  
32비트-> 128비트로 늘어나게 됨.  
즉 이진수 32자리가 128자리가 됨.

하지만 IPv4, 6는 상호 호환성 없어서 IPv4전제로 한 네트워크 장비나 애플리케이션은 IPv6이용 불가.  
상호 통신 위해서는 기본적으로 웹 서비스 제공 측과 이용 측 쌍방이 IPv6대응 네트워크 장비와 애플리케이션 이용할 필요 있다.

TCP 위의 레이어인 HTTP는 두 버전의 IP에 모두 대응해 동작하기 때문에 사용자는 Ipv4인지 6인지 의식할 필요없다.  
-> 계층 구조의 장점.

### 6.7.4 라우팅

IP 주소를 이용해 지정한 대상 서버가 다른 네트워크에 있다면 최종 목적지 도착까지 목적지 알고 있는 라우터에게 전송 부탁하게 된다.

- IP 패킷 받은 라우터는 해당 IP 패킷의 헤더에서 목적지 확인 뒤 어디로 보낼지 알아낸다.  
  -> 이때 사용하는 것이 라우팅테이블(경로표)

- 외부와 접속하는 네트워크에는 보통 `게이트웨이(default gateway)`라는 라우터가 설치돼 있다.
  ex) 네트워크 A,B,C 있다고치면

1. A, B에 속한 라우터가 C의 IP패킷을 받으면 해당 주소가 어딘지 알 수없다. (자신이 가진 라우팅 테이블에 C네트워크 없다고 가정)
2. 외부와 연결되 있는 게이트웨이 라우터로 IP패킷을 보냄
3. 게이트웨이 라우터는 자신이 아는 범위내에서 IP패킷이 목적지에 다다를 때까지 다른 라우터로 IP패킷을 라우팅  
   -> 여기서 경로 내 특정 라우터의 라우팅 테이블에 오류가 발생한 경우 등 네트워크 패킷이 계속 순회하게 되서 곤란한 상태에 이르는 경우가있음.  
   -> 이를 방지하기위해 IP 헤더는 TTL(Time to Live)이라는 생존시간 정보를 갖는다.  
   -> IP헤더의 TTL=64 부터 시작한다고 치면 라우터 하나 경유할때마다 TTL이 1씩 차감 됨

## 6.8 레이어2 데이터 링크 계층의 프로토콜 이더넷

링크계층에 사용되는 대표적 프로토콜 이더넷(ethernet)  
링크계층 사용되는 프로토콜은 여러가지임. 이더넷, 무선랜 등

링크 계층 프로토콜의 역할: `동일 네트워크 내`의 네트워크 장비까지 전달받은 데이터를 운반

TCP/IP 4계층 모델 에서는 물리 계층과 함께 하나의 계층으로 취급 됨.

- IP는 IP주소 이용 여러 네트워크를 거쳐 데이터 전송 가능하지만, 이더넷은 동일 네트워크 즉, 자신이 포함된 링크 내에서만 데이터 전송가능.  
  ->이때 사용되는 주소가 MAC(맥)주소

### 이더넷 처리 흐름

1. IP 패킷에 이더넷 `헤더와 푸터`를 붙여서 `이더넷 프레임`을 만든다.
2. 헤더에는 링크계층 주소인 MAC주소 기록되 있다.
3. 목적지 IP주소에 따라 경로표(라우팅테이블) 이용해서 어떤 NIC에서 보내야 할 지 결정.
4. 송신큐에 넣은 후 장치드라이버 경유로 NIC에 전달

MAC주소에는 IP주소의 라우팅 테이블 처럼 ARP테이블(MAC 테이블)이라는 것이 있다.  
-> IP주소 A에 대응하는 것은 MAC주소 A다 와 같은 형식

- 하나의 이더넷 프레임으로 전송할 수 있는 최대 크기 MTU(Maximum Transfer Unit)라 부름.
- 일반적인 이더넷 에서는 1500바이트로 설정돼 있다.

- `MTU`에서 IP 및 TCP 헤더 크기 뺀 것이 `TCP의 MSS`

- IP패킷 최대 크기 MTU 보다 작으면 하나의 패킷으로 전달할 수있는 데이터 크기가 작아 큰 데이터 전달 위해 통신 횟수가 늘어남. (핑퐁 - 교환횟수가 늘어나면 통신이 느려짐)

- IP 패킷의 최대크기 MTU 보다 크면 IP계층에서 MTU사이즈에 맞춰 패킷분할이 이루어짐  
  ->TCP 계층에서 여러 세그먼트로 분할된 후 다시 IP 계층에서도 분할이 이루어지는것. (이중작업으로 불필요한 처리시간 늘어난다.)

- MTU는 네트워크 상의 모든 경로에서 동일하지 않다.  
  -> 경로 도중 MTU가 작아 1패킷이 MTU에 들어가지 않으면 필요에 따라 경로 도중 네트워크 장비에서 패킷이 분할됨.  
  -> 환경설정에 따라 패킷이 파기되기도 함.  
  -> IPv4는 패킷 분할을 하지만 IPv6에서는 폐지 됨

### 6.8.3 동일 네트워크 내의 데이터 전송

MAC 주소는 네트워크 통신하는 하드웨어에 할당된 주소.  
원칙적으로 세상의 모든 장비가 고유한 물리 주소를 갖게하자는 개념이지만 실제로는 설정 등 통해 MAC주소 겹치기도 함

1. 서버 등이 보낸 이더넷 프레임 L2스위치에 도착
2. L2스위치는 MAC주소 보고 적절한 포트에서 프레임 꺼냄
3. 다른 네트워크 (L3 스위치나 라우터)거치는 경우 MAC주소 사용한 통신 불가능.
4. IP 이용한 브로드 캐스트 주소 통신은 이더넷 상에서의 브로드 캐스트 통신으로 전송 됨.
   -> MAC주소 FF-FF-FF-FF-FF가 이더넷 브로드 캐스트에 해당.
